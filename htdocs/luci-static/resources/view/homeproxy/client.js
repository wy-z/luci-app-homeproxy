...
function renderStatus(isRunning, port) {
	...
	if (isRunning) {
		...
		renderHTML =
			spanTemp.format("green", _("HomeProxy"), _("RUNNING")) + button;
	} else renderHTML = spanTemp.format("red", _("HomeProxy"), _("NOT RUNNING"));
...
function validatePortRange(section_id, value) {
	if (section_id && value) {
		...
		if (value && (value[1] || value[2])) {
			...
			else if (!value[2]) value[2] = 65535;

			if (value[1] < value[2] && value[2] <= 65535) return true;
		}
...
return view.extend({
	...
	render: function (data) {
		...
		uci.sections(data[0], "node", (res) => {
			...
			proxy_nodes[res[".name"]] = String.format(
				"[%s] %s",
				res.type,
				res.label ||
				(stubValidator.apply("ip6addr", nodeaddr)
					? String.format("[%s]", nodeaddr)
					: nodeaddr) +
				":" +
				nodeport,
			);
		...
		for (var i in proxy_nodes) o.value(i, proxy_nodes[i]);
		o.default = "nil";
		...
		o.validate = function (section_id, value) {
			if (section_id && !["wan"].includes(value)) {
				var ipv6_support = this.map
					.lookupOption("ipv6_support", section_id)[0]
					.formvalue(section_id);

				if (!value) return _("Expecting: %s").format(_("non-empty value"));
				else if (
					!stubValidator.apply(
						ipv6_support === "1" ? "ipaddr" : "ip4addr",
						value,
					)
				)
		...
		if (features.hp_has_chinadns_ng) {
			...
			o.validate = function (section_id) {
				if (section_id) {
					var value = this.map
						.lookupOption("china_dns_server", section_id)[0]
						.formvalue(section_id);
					...
					for (var dns of value) {
						...
						if (dns === "wan") {
							...
							if (
								...
								return err;
							if (dns[1] && !stubValidator.apply("port", dns[1])) return err;
						}
					}
				}
		...
		o.onchange = function (ev, section_id, value) {
			if (section_id && value === "custom") this.map.save(null, true);
		};
		...
		o = s.taboption(
			...
			_("Routing ports"),
			_(
				"Specify target ports to be proxied. Multiple ports must be separated by commas.",
			),
		);
		...
		o.validate = function (section_id, value) {
			if (section_id && value !== "all" && value !== "common") {
				if (!value) return _("Expecting: %s").format(_("valid port value"));

				var ports = [];
				for (var i of value.split(",")) {
					if (
						!stubValidator.apply("port", i) &&
						!stubValidator.apply("portrange", i)
					)
						return _("Expecting: %s").format(_("valid port value"));
					if (ports.includes(i)) return _("Port %s alrealy exists!").format(i);
		...
		if (features.hp_has_ip_full && features.hp_has_tun) {
			...
			o.description = _(
				"To enable Tun support, you need to install <code>ip-full</code> and <code>kmod-tun</code>",
			);
		}
		...
		so.onchange = function (ev, section_id, value) {
			...
			if (value === "mixed")
				desc.innerHTML = _(
					"Mixed <code>system</code> TCP stack and <code>gVisor</code> UDP stack.",
				);
			else if (value === "gvisor")
				desc.innerHTML = _("Based on google/gvisor.");
			else if (value === "system")
				desc.innerHTML = _(
					"Less compatibility and sometimes better performance.",
				);
		};
		...
		so = ss.option(
			...
			_("Enable endpoint-independent NAT"),
			_(
		...
		so.load = function (section_id) {
			...
			uci.sections(data[0], "routing_node", (res) => {
				if (res.enabled === "1") this.value(res[".name"], res.label);
			});
		...
		ss.modaltitle = L.bind(
			hp.loadModalTitle,
			this,
			_("Routing node"),
			_("Add a routing node"),
			data[0],
		);
		...
		so = ss.option(form.ListValue, "node", _("Node"), _("Outbound node"));
		for (var i in proxy_nodes) so.value(i, proxy_nodes[i]);
		so.validate = L.bind(
			hp.validateUniqueValue,
			this,
			data[0],
		...
		so = ss.option(
			...
			_("Domain strategy"),
			_(
				"If set, the server domain name will be resolved to IP before connecting.<br/>dns.strategy will be used if empty.",
			),
		);
		for (var i in hp.dns_strategy) so.value(i, hp.dns_strategy[i]);
		so.modalonly = true;
		...
		so = ss.option(
			...
			_("Outbound"),
			_(
				"The tag of the upstream outbound.<br/>Other dial fields will be ignored when enabled.",
			),
		);
		so.load = function (section_id) {
			delete this.keylist;
		...
		so.validate = function (section_id, value) {
			if (section_id && value) {
				var node = this.map
					.lookupOption("node", section_id)[0]
					.formvalue(section_id);
				...
				if (conflict) return _("Recursive outbound detected!");
			}
		...
		so = ss.option(
			...
			_("Mode"),
			_(
				"The default rule uses the following matching logic:<br/>" +
		...
		so = ss.option(
			...
			_("Protocol"),
			_(
				'Sniffed protocol, see <a target="_blank" href="https://sing-box.sagernet.org/configuration/route/sniff/">Sniff</a> for details.',
			),
		);
		...
		so = ss.option(
			form.DynamicList,
			"source_geoip",
			_("Source GeoIP"), // TODO: deprecated
		...
		so = ss.option(form.DynamicList, "port", _("Port"), _("Match port."));
		so.datatype = "port";
		so.modalonly = true;
		...
		so = ss.option(form.DynamicList, "user", _("User"), _("Match user name."));
		so.modalonly = true;
		...
		so = ss.option(
			...
			_("Rule set IPCIDR match source"),
			_("Make <code>ipcidr</code> in rule sets match the source IP."),
		);
		so.default = so.disabled;
		so.modalonly = true;

		so = ss.option(form.Flag, "invert", _("Invert"), _("Invert match result."));
		so.default = so.disabled;
		so.modalonly = true;
		...
		so = ss.option(form.Value, "path", _("Path"), _("File path of Rule Set."));
		so.datatype = "file";
		...
		so = ss.option(form.Value, "url", _("URL"), _("Download URL of Rule Set."));
		so.validate = function (section_id, value) {
			if (section_id && value) {
				try {
		...
		so = ss.option(
			form.Flag,
			"disable_cache_expire",
			_("Disable cache expire"),
		);
		so.default = so.disabled;
		...
		so = ss.option(
			form.Flag,
			"independent_cache",
			_("Independent cache per server"),
			_(
		...
		so = ss.option(
			...
			_("Address resolver"),
			_(
				"Tag of a another server to resolve the domain name in the address. Required if address contains domain.",
			),
		);
		so.load = function (section_id) {
			delete this.keylist;
		...
		so.validate = function (section_id, value) {
			if (section_id && value) {
				...
				if (conflict) return _("Recursive resolver detected!");
			}
		...
		so = ss.option(
			...
			_("Address strategy"),
			_(
				"The domain strategy for resolving the domain name in the address. dns.strategy will be used if empty.",
			),
		);
		for (var i in hp.dns_strategy) so.value(i, hp.dns_strategy[i]);
		so.modalonly = true;
		...
		so.depends({
			lan_proxy_mode: "except_listed",
			"homeproxy.config.ipv6_support": "1",
		});

		so = fwtool.addMACOption(
			ss,
			"lan_ip_policy",
		...
		so = fwtool.addIPOption(
			ss,
			"lan_ip_policy",
			"lan_gaming_mode_ipv4_ips",
			_("Gaming mode IPv4 IP-s"),
			null,
			"ipv4",
			hosts,
			true,
		...
		so = fwtool.addIPOption(
			ss,
			"lan_ip_policy",
			"lan_gaming_mode_ipv6_ips",
			_("Gaming mode IPv6 IP-s"),
			null,
			"ipv6",
			hosts,
			true,
		...
		so = fwtool.addIPOption(
			ss,
			"lan_ip_policy",
			"lan_global_proxy_ipv4_ips",
			_("Global proxy IPv4 IP-s"),
			null,
			"ipv4",
			hosts,
			true,
		...
		so = fwtool.addIPOption(
			ss,
			"lan_ip_policy",
			"lan_global_proxy_ipv6_ips",
			_("Global proxy IPv6 IP-s"),
			null,
			"ipv6",
			hosts,
			true,
		...
		so = fwtool.addMACOption(
			ss,
			"lan_ip_policy",
			"lan_global_proxy_mac_addrs",
			_("Global proxy MAC-s"),
			null,
			hosts,
		);
		so.depends({ "homeproxy.config.routing_mode": "custom", "!reverse": true });
